#' @export
surrogate_GP_2D <- function(f, data, method = c("ML", "VB"), layers = layers,
  ...
  if (model == "meanonly"){
    if(method == "ML") {
      NMLL <- logmarglik_2D_warp(s_in = swarped_final,
                                 Sobs_tf = Sobs_tf_1,
                                 l_tf = l_tf,
                                 sigma2_tf = sigma2_tf,
                                 z_tf = z_tf_1)
  ...
  if (model == "meanvar"){
  if(method == "ML") {
    NMLL <- logmarglik_d_2D1_warp(s_in = swarped_final,
                                 Sobs_tf = Sobs_tf,
                                 l_tf = l_tf,
                                 sigma2_tf = sigma2_tf,
                                 z_tf = z_tf,
                                 swarped_d_v1 = swarped_final_d,
                                 swarped_d2_v1 = swarped_final_d2)
    Cost <- NMLL$Cost
  }
  }
 ...
 #}
  ...
  class(deepspat.obj) <- "surrogate_GP_2D"
  deepspat.obj
}

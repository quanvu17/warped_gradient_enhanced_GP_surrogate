synthetic_likefnc_GP_2D_stat <- function(object1, object2, newdata, summary_stats, mean1, mean2){
  ...
  K_obs_star <- cov_matern_tf_2D(x1 = obs_swarped, x2 = newdata_swarped, sigma2f = d1$sigma2_tf, alpha = tf$reciprocal(d1$l_tf))
  K_obs_star_2 <- cov_matern_tf_2D(x1 = obs_swarped, x2 = newdata_swarped_2, sigma2f = d1$sigma2_tf, alpha = tf$reciprocal(d1$l_tf))
  ...
  newdata_swarped_2 <- s_in_2
    ...
    K_obs_star <- cov_matern_tf_2D(x1 = obs_swarped, x2 = newdata_swarped, sigma2f = d2$sigma2_tf, alpha = tf$reciprocal(d2$l_tf))
    K_obs_star_2 <- cov_matern_tf_2D(x1 = obs_swarped, x2 = newdata_swarped_2, sigma2f = d2$sigma2_tf, alpha = tf$reciprocal(d2$l_tf))
    ...
    pred_mean_2_2 <- tf$matmul(tf$matrix_transpose(Kobs_chol_star_2), Kobs_chol_z)
    
  
  pred_mean_2 <- d2$run(pred_mean_2) + mean2
  pred_mean_2_2 <- d2$run(pred_mean_2_2) + mean2
  
  pred_var_2 <- pmax((pred_mean_2_2 - pred_mean_2) / 1e-5, rep(1e-5, length(pred_mean_2)))
  pred_sd_2 <- sqrt(pred_var_2)
  
  z1 <- summary_stats[1]
  z2 <- summary_stats[2]
  
  Cost1 <- - log(pred_sd_1) - 1/2 * (z1 - pred_mean_1)^2 / pred_var_1
  Cost2 <- - log(pred_sd_2) - 1/2 * (z2 - pred_mean_2)^2 / pred_var_2
  syn_like <- Cost1 + Cost2
  
  pred <- list(pred_mean_1, pred_mean_2, pred_var_1, pred_var_2, syn_like)

### Delayed-acceptance MCMC with the hidden Potts model
...
for (i in 2:nMC){
  ...
  print(paste(i, beta[i], sumZs[i], acc_step1, acc))
}
t2 <- proc.time()
time_total <- t2 - t1
time_burnin <- t3 - t1

delay_accept <- list(beta, muEst, sdEst, sumZs, z, acc, acc_step1, time_total, time_burnin)
save(delay_accept, file = 'results/hidden_potts_delayed_accept.rda')
